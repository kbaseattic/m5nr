...
foreach my $resource (@{$struct->{resources}}){
  ...
  foreach my $request (@{$resource->{requests}}){
    ...
    my $func_name = $method."_".$resource->{name}."_".$request->{name};
    ...
    $func .= $func_name."(".$func_name."_params) returns (";
    ...
    $pod_string .= "\n=back\n\n";
    $definition_string .= "\t} ".$func_name."_params;\n";
...
sub parse_types {
  ...
  foreach my $key (keys(%$params)) {
	...
	if ($simple_types->{$params->{$key}->[0]}) {
	  push(@types, [ $simple_types->{$params->{$key}->[0]}." $key", $params->{$key}->[1] ]);
	} else {
	  my $tt;
	  if ((undef, $tt) = $params->{$key}->[0] =~ /^(list)\s(.+)$/) {
	    my $type = "list<";
	    my $closer = ">";
	    my $t;
	    while ((undef, $t) = $tt =~ /^(list)\s(.+)$/) {
	      $type .= "list<";
	      $closer .= ">";
	      $tt = $t;
	    }
	    if ($simple_types->{$tt}) {
	      push(@types, [ $type.$simple_types->{$tt}.$closer." $key", $params->{$key}->[1] ]);
	    } else {
	      if ($verbose) {
	    ...
	    if ($verbose) {
	      print "attribute $key has and invalid type, skipping.\n";
	    }
	  }
	}
      } else {
	if ($verbose) {
	  print "attribute $key has no data type, skipping.\n";
	}
      }
    } else {
      if ($verbose) {
  ...
  foreach my $key (keys(%$params)) {
	...
	if ($valid_types->{$param_desc->[0]}) {
	  if ($param_desc->[0] eq 'list') {
	    if ($param_desc->[1]) {
	      if ($valid_types->{$param_desc->[1]}) {
		if ($param_desc->[1] eq 'list') {
		  if ($verbose) {
		    print "the element data type of the list of parameter $key is a list, but lists may not be nested, skipping.\n";
		  }
		} else {
		  push(@parameters, "list<".$valid_types->{$param_desc->[1]}."> $key;");
		}
	      } else {
		if ($verbose) {
		  print "the element data type of the list of parameter $key has an invalid type ".$param_desc->[1].", skipping.\n";
		}
	      }
	    } else {
	      if ($verbose) {
